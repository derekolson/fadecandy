...
int fast_build_block_buffer(struct mpsse_context *mpsse, uint8_t cmd, unsigned char *data, int size, int *buf_size)
{
	...
	*buf_size = 0;
	...
	mpsse_fast_rw_buf[i++] = cmd;
	mpsse_fast_rw_buf[i++] = (rsize & 0xFF);
	mpsse_fast_rw_buf[i++] = ((rsize >> 8) & 0xFF);

	/* On a write, copy the data to transmit after the command */
	if((cmd == mpsse->tx || cmd == mpsse->txrx) && (i + size) <= sizeof(mpsse_fast_rw_buf))
	{
		memcpy(mpsse_fast_rw_buf+i, data, size);
	...
	if(is_valid_context(mpsse))
	{
		if(mpsse->mode)
		{
			while(n < size)
			{
				...
				{	
					if(raw_write(mpsse, mpsse_fast_rw_buf, buf_size) == MPSSE_OK)
					{
						n += txsize;
					}
					else
	...
	if(is_valid_context(mpsse))
	{
		if(mpsse->mode)
		{
			while(n < size)
			{
				...
				{
					if(raw_write(mpsse, mpsse_fast_rw_buf, data_size) == MPSSE_OK)
					{
