...
{
	return mpsse_open_index(vid, pid, mode, freq, endianess, interface, description, serial, 0);
}
...
struct mpsse_context *mpsse_open_index(int vid, int pid, enum modes mode, int freq, int endianess, int interface, const char *description, const char *serial, int index)
{
	...
	if(mpsse)
	{
		...
		mpsse_flush_after_read(mpsse, 0);
					...
					if(mpsse->mode != BITBANG)
					{
						...
						if(mpsse_set_clock(mpsse, freq) == MPSSE_OK)
						{
							if(mpsse_set_mode(mpsse, endianess) == MPSSE_OK)
							{
								...
								/* 
								 * Not all FTDI chips support all the commands that mpsse_set_mode may have sent.
...
struct mpsse_context *mpsse_open_dev(struct libusb_device *dev, int freq)
{
	...
	if(mpsse)
	{
		...
		{
			/* Set the FTDI interface  */
			ftdi_set_interface(&mpsse->ftdi, INTERFACE_A);

			/* Open the specified device */
			int r = ftdi_usb_open_dev(&mpsse->ftdi, dev);
			...
			if(r == 0)
			{
				...
				if(status == 0)
				{
					...
					if(mpsse->mode != BITBANG)
					{
						...
						if(mpsse_set_clock(mpsse, freq) == MPSSE_OK)
						{
							if(mpsse_set_mode(mpsse, MSB) == MPSSE_OK)
							{
								mpsse->open = 1;
...
int mpsse_set_mode(struct mpsse_context *mpsse, int endianess)
{
	...
	/* Do not call is_valid_context() here, as the FTDI chip may not be completely configured when mpsse_set_mode is called */
	if(mpsse)
	{
...
int mpsse_set_clock(struct mpsse_context *mpsse, uint32_t freq)
{
	...
	/* Do not call is_valid_context() here, as the FTDI chip may not be completely configured when mpsse_set_clock is called */
	if(mpsse)
	{
		if(freq > SIX_MHZ)
		{
...
int mpsse_write_bits(struct mpsse_context *mpsse, char bits, int size)
{
	...
	mpsse_enable_bitmode(mpsse, 1);
	retval = mpsse_write(mpsse, data, size);
	mpsse_enable_bitmode(mpsse, 0);
...
/* Performs a read. For internal use only; see Read() and mpsse_read_bits(). */
char *mpsse_internal_read(struct mpsse_context *mpsse, int size)
{
...
char *mpsse_read(struct mpsse_context *mpsse, int size)
{
	...
	buf = mpsse_internal_read(mpsse, size);
	return buf;
}
...
char mpsse_read_bits(struct mpsse_context *mpsse, int size)
{
	...
	mpsse_enable_bitmode(mpsse, 1);
	rdata = mpsse_internal_read(mpsse, size);
	mpsse_enable_bitmode(mpsse, 0);
 ...
 * @mpsse - MPSSE context pointer.
 * @pin   - The pin number.
 * @state - The state of the pins, as returned by mpsse_read_pins.
 *          If set to -1, mpsse_read_pins will automatically be called.
 *
